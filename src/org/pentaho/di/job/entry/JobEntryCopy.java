...
public class JobEntryCopy implements Cloneable, XMLInterface, GUIPositionInterface, ChangedFlagInterface,
  ...
  //thangtq
  private boolean exeFinalResult=false;
  
  public void setExeResult(boolean exeresult)
  {
      exeFinalResult=exeresult;
  }
  public boolean getExeResult()
  {
      return exeFinalResult;
  }
  ...
  public String toString() {
    if ( entry != null ) {
      ...
      return entry.getName() + "." + getNr() + " id:" + id + " isJob:"+isJob() 
              + " isStart:"+isStart() + " exeFinalResult:"+exeFinalResult + calcElapseTime();
    } else {
      return "null." + getNr();
    }
  }
   ...
   * @return 
   */
   public String calcElapseTime() 
   {
    if ( entry != null ) 
        if(isJob())
        {
            Job job=((JobEntryJob)entry).getJob();
            if(job != null)
            return job.toStringJobElapsedTime();
            else 
            {
                Job parentJob=((JobEntryJob)entry).getParentJob() ;
                if(parentJob!= null)
                    return  " parentJob:"+ parentJob.toStringJobElapsedTime();
            }
        }
